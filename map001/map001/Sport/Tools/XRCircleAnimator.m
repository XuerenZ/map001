...
@interface XRCircleAnimator() <UIViewControllerAnimatedTransitioning, CAAnimationDelegate>

@end

@implementation XRCircleAnimator {
    /// 是否展现标记
    BOOL _isPresented;
...
- (void)circleAnimWithView:(UIView *)view {
    ...
    // 设置动画代理 - 如果将动画添加到图层，动画会立即开始，然后再设置代理，将无效！
    // anim.delegate = self;
}
