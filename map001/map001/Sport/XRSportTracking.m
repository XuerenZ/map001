...
/// GPS 信号变化通知
NSString *const XRSportGPSSignalChangedNotification = @"XRSportGPSSignalChangedNotification";
...
- (XRSportPolyline *)appendLocation:(CLLocation *)location {
    ...
    // 判断 GPS 信号
    if ([self gpsSignalWithLocation:location] < XRSportGPSSignalStateNormal) {
        return nil;
    }
...
/**
 ...
 @return 返回 GPS 状态
 */
- (XRSportGPSSignalState)gpsSignalWithLocation:(CLLocation *)location {
    
    XRSportGPSSignalState state = XRSportGPSSignalStateBad;
    ...
    // 3> 根据时间差值计算 GPS 强度枚举
    if (delta < 0.01) {
        state = XRSportGPSSignalStateGood;
    } else if (delta < 1) {
        state = XRSportGPSSignalStateNormal;
    }
 ...
 */
- (void)postNotifyWithState:(XRSportGPSSignalState)state {
    
    [[NSNotificationCenter defaultCenter] postNotificationName:XRSportGPSSignalChangedNotification object:@(state)];
}
